    ...
    <div>
      ...
      >
        {moreActions.map((action) => {
          ...
          return (
            <MenuItem key={value} onClick={() => handler(value)}>
              {label}
            </MenuItem>
          );
        })}
...
const AuditList = ({ classes, data, handleAudit, vulnerabilities }) => {
  ...
  return (
    <Grid container spacing={2}>
      <Grid item sm={12} md={8} lg={8} xl={8} className={classes.transition}>
              ...
              <TableBody>
                {Object.values(data).map(
                  (
                    ...
                    >
                      <TableCell
                        className={cn(classes.tableCell, classes.text)}
                      >
...
AuditList.propTypes = {
  ...
  vulnerabilities: PropTypes.objectOf(
