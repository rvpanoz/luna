...
const handlers = {
  ...
  [SET_ROWS_PER_PAGE]: (state, action) => R.assoc("rowsPerPage", action.rows, state),
  [SET_VERSION]: (state, action) => R.assoc("version", action.version, state),
  [SET_TAB_INDEX]: (state, action) => R.assoc("tabIndex", action.tabIndex, state),
  ...
  [ADD_COMMAND_OPTION]: (state, action) => {
    ...
    return R.merge(state, {
      cmdOptions:
        idx !== -1 ? R.remove(idx, 1, state.cmdOptions) : R.prepend(action.option, state.cmdOptions)
    });
  },
